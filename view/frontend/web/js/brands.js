...
define(['jquery', 'jquery-ui-modules/widget', 'mr_owlslider'], function($, owlCarousel, domReady) {
    var rtl = $('body').hasClass('rtl-layout');
    var mBreakpoint = 768;

    function itemEffect($parent, delayUnit) {
        $('.hkk-transparent', $parent).each(function(i, el) {
            var $item = $(el);
            setTimeout(function() {
                $item.removeClass('hkk-transparent').addClass('hkk-translator');
                setTimeout(function() {
                    $item.removeClass('hkk-translator');
                }, 1000);
            }, delayUnit * i);
        });
    }
    $.widget('magerubik.brandslider', {
        options: {
            mbMargin: 10
        },
        _create: function() {
            ...
            if (typeof conf.sliderConfig.responsive !== 'undefined') {
                $.each([768, 480, 320, 0], function(i, breakPoint) {
                    if (typeof conf.sliderConfig.responsive[breakPoint] !== 'undefined') {
                        conf.sliderConfig.responsive[breakPoint] = $.extend({}, {
                            margin: conf.mbMargin
                        }, conf.sliderConfig.responsive[breakPoint]);
                    }
                });
            }
    ...
    $.widget('magerubik.alphabetList', {
        ...
        _sameHeight: function() {
            ...
            self.element.find('.brand-group').each(function() {
                var maxHeight = 0,
                    $group = $(this);
                $group.find(conf.sameHeight).css({
                    minHeight: ''
                }).each(function() {
                    var $sItem = $(this);
                ...
                success: function(res) {
                    require(['jquery-ui-modules/autocomplete'], function() {
                        ...
                        uiAutocomplete._renderItem = function(ul, item) {
                            ...
                            html += '</a>';
                            return $('<li class="item">').append(html).appendTo(ul);
                        };
    ...
    $.widget('magerubik.autowidth', {
        ...
        _sameHeight: function() {
            ...
            $.each(conf.sameHeight, function(i, sameHeight) {
                self.element.find(sameHeight).css({
                    minHeight: ''
                }).each(function() {
                    var $sItem = $(this);
                    var height = $sItem.outerHeight();
        ...
        _create: function() {
            ...
            $('body').on('contentUpdated', function() {
                var itemClass = 'hkk-grid-item-' + self.gridId;
