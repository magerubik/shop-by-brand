...
define([
        ...
        openDialog: function(btn, url, width, height, title, options) {
            ...
            if (!this.modal) {
                ...
                $.ajax({
                    ...
                    self.modal.html(data.replace('"mediabrowser"','"Magerubik_Shopbybrand/js/browser"')).trigger('contentUpdated');
                });
            } else {
                this.modal.modal('openModal');
            }
    ...
    $.widget("custom.hkkMediabrowser", {
        eventPrefix: "hkkMediabrowser",
        options: {
        ...
        insert: function(event) {
            ...
            if (!targetEl.length) {
                HkkMediabrowserUtility.closeDialog();
            ...
            return $.ajax({
                ...
                if (typeof data == 'string') {
                    ...
                    if (targetEl.data('force_static_path')) {
                        ...
                        data.full = Magerubik.mediaUrl + img.substr('/media/'.length);
                    } else {
                        ...
                        if (!data.short) {
                            img.gsub(/src="(.*?)"/i, function (match) {
                                var src = '';
                                data.full = match[1];
                                data.short = match[1].replace(Magerubik.mediaUrl, '');
                            });
                        }
                        data.full = Magerubik.mediaUrl + data.short;
                    }
                }
        ...
        getTargetElement: function() {
            if (typeof HkkMediabrowserUtility.targetElement !== 'undefined') {
                return HkkMediabrowserUtility.targetElement;
            } else {
